...
generate_ignite_configs() {
	...
	sudo docker run --rm -it \
	       ...
	       hkjn/golang:1.9.2 go run generate_ignite_configs.go
}
...
#	           -v $(pwd):/home/go/src/hkjn.me/src/infra \
#	           -w /home/go/src/hkjn.me/src/infra \
#	       hkjn/golang:1.9.2 go run fetch_checksums.go
# }
...
run_tf() {
	...
	echo "infra_version = \"$(cat VERSION)\"" > terraform.tfvars
	...
	sudo docker run --rm -it -v $(pwd):/home/tfuser \
	           -e GOOGLE_APPLICATION_CREDENTIALS=/home/tfuser/.gcp/tf-dns-gcp-admin.json \
		   hkjn/terraform:0.12.1 $@
}
